...
window.addEventListener("load", function () {
  ...
  let next = document.querySelector(".next-dir");
  let prov = document.querySelector(".prev-dir");
  ...
  btnnext.addEventListener("click", function (e) {
    if (index < sliderLeng - 1) {
      ...
      position -= widthSliderItem;
      wrapImg.style = `transform: translateX(${position}px)`;
      if (index > 0 && index < sliderLeng) {
        console.log("index:", index);
        prov.style.color = "#222222";
      } else if (index === sliderLeng - 1) {
        prov.style.color = "#999999";
      }
    }
  });
});
